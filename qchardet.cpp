...
QTextCodec *QChardet::encodingForByteArray(QByteArray data,float* confidence)
{
    ...
    {
        char *charData = new char[static_cast<uint>(data.size() + 1)];
        strcpy(charData, data.data());
        switch (detect_r(charData,strlen(charData),&detectObj))
        {
        case CHARDET_OUT_OF_MEMORY :
            qCritical() << "libchardet: On handle processing, occured out of memory\n";
            detect_obj_free (&detectObj);
            return nullptr;
        case CHARDET_NULL_OBJECT :
            qCritical() << "libchardet: 2st argument of chardet() is must memory allocation with detect_obj_init API\n";
            return nullptr;
        }
        if (confidence)
